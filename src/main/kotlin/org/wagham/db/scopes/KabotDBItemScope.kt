...
class KabotDBItemScope(
	...
	fun getCharactersWithItem(guildId: String, item: String, onlyActive: Boolean): Flow<Character> = client.getGuildDb(guildId)
		.getCollection<Character>(CollectionNames.CHARACTERS.stringValue)
		.find(
			*listOfNotNull(
				(Character::status eq CharacterStatus.active).takeIf { onlyActive },
