...
class KabotDBItemScopeTest : StringSpec() {
	...
	private fun StringSpec.testItems() {
		...
		"Can get all the characters with a specific item in the inventory" {
			val item = client.charactersScope.getAllCharacters(guildId)
				.filter { it.status == CharacterStatus.active }
				.take(100).toList()
				.random().inventory.keys.random()
			val characters = client.charactersScope.getAllCharacters(guildId).filter {
				it.status == CharacterStatus.active && it.inventory.containsKey(item)
			}.toList()

			val result = client.itemsScope.getCharactersWithItem(guildId, item, true).toList()
			result shouldContainExactlyInAnyOrder characters
		}
	}
}